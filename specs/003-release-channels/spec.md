...
- Q: `instanceReason` is recorded on whichever copy was installed second, so installing on `rc` first and `stable` second labelled the **stable** copy `stable copy of gitea` and left the rc copy unlabelled ([#433](https://github.com/try-hola/hola/issues/433)). Should the reason move, or the label change? → A: **The label changes; the record does not.** `instanceReason` stays the install-time audit fact (recomputing it would make an audit record a live one, and [#431](https://github.com/try-hola/hola/issues/431) depends on it as written). The detail instead exposes the app's other live deployments and the dashboard derives the label from this copy's own channel plus those siblings — accurate on both copies in either order — appending the reason as a secondary "permitted by …" phrase when one was recorded. FR-016 and FR-022 amended.
...
- **FR-016** *(amended 2026-09-04, [#431](https://github.com/try-hola/hola/issues/431); amended 2026-09-04, [#433](https://github.com/try-hola/hola/issues/433))*: A second copy of a single-instance app MUST record the **instance reason** that permitted it: `channel` when FR-015's channel rule is what permitted it — no existing copy followed its channel *and* that channel is published (this takes precedence even if the override was also supplied) — otherwise `operator-override`. First copies and copies of multi-instance apps record no reason. The reason MUST be exposed on the deployment detail. It is an **install-time audit fact** about the install that was permitted, recorded once and never recomputed — it is not a description of the copy, so it MUST NOT be the sole basis of the detail's instance label (see FR-022). The deployment detail MUST additionally expose the app's other live deployments (id, name, channel), derived at read time and absent when there are none.
...
- **FR-022** *(amended 2026-09-04, [#433](https://github.com/try-hola/hola/issues/433))*: The deployments list and the deployment detail MUST show a channel badge for non-stable deployments; the detail MUST show the channel and, whenever the app has more than one live copy, an instance label naming **this** copy's channel and the app's other copies with theirs — on every copy, independent of install order. When an instance reason was recorded, the label MUST append it as a secondary phrase (permitted by channel / by operator override).
