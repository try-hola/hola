...
def list_servers():
    ...
    if not servers:
        format_output(
            "No servers found. Use 'hola servers create' to create one.",
            style="info"
        )
        return
...
def start_server(
    ...
    async def _start_server():
        try:
            ...
            format_output(
                f"Starting server '{server.name}' ({server_id})...",
                style="info"
            )
            ...
            if updated_server and updated_server.status == ServerStatus.RUNNING:
                format_output(
                    f"Server '{server.name}' started successfully.",
                    style="success"
                )
            elif updated_server:
                format_output(
...
def stop_server(
    ...
    async def _stop_server():
        try:
            ...
            format_output(
                f"Stopping server '{server.name}' ({server_id})...",
                style="info"
            )
            ...
            if updated_server.status == ServerStatus.STOPPED:
                format_output(
                    f"Server '{server.name}' stopped successfully.",
                    style="success"
                )
            else:
                format_output(
...
def delete_server(
    ...
    format_output(
        f"Server record for '{server.name}' deleted successfully.",
        style="success"
    )
