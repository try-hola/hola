...
class FakeProvider:
    ...
    def create_server(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "id": f"{self.provider_type}-server-123",
            "name": config.get("server_name", "test-server"),
            "status": "running",
...
class FakeProviderRegistry:
    ...
    def add_fake_provider(
        self, provider_type: str, name: Optional[str] = None
    ) -> FakeProvider:
        provider = FakeProvider(
            provider_type=provider_type, name=name or f"fake-{provider_type}-provider"
        )
