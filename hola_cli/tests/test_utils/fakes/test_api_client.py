...
def test_fake_api_client_set_response_with_text_and_headers():
    ...
    client.set_response("POST", "/submit", {"id": 1}, 201, response_text="Created", response_headers=headers)
...
def test_fake_api_client_sync_detailed_request_logging():
    ...
    client.sync_detailed("POST", "/log_me", json=payload, params=params, headers=headers, custom_kwarg="custom")
...
def test_fake_response_initialization():
    response = FakeResponse(status_code=200, data={"key": "value"}, text="raw_text", headers={"X-Test": "TestVal"})
    assert response.status_code == 200
...
def test_fake_response_json_method_success():
    response = FakeResponse(status_code=200, data={"key": "value"}, headers={"Content-Type": "application/json"})
    assert response.json() == {"key": "value"}


def test_fake_response_json_method_failure_content_type():
    response = FakeResponse(status_code=200, data={"key": "value"}, headers={"Content-Type": "text/plain"})
