...
def test_get_current_server_explicit_name_override(mock_environment):
    ...
    settings = {
        "servers": {
            "test-server": {"url": "http://test-server-url", "api_key": "test-server-key"},
            "explicit-server": {"url": "http://explicit-url", "api_key": "explicit-key"}
        },
        "default_server": "test-server"
    }
    
    with patch("hola_cli.config.context.load_settings", return_value=CliSettings.model_validate(settings)):
