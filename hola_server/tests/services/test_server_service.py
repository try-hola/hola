...
from hola_shared.models import ServerStatus, ServerState, HealthStatus, VersionInfo, ResourceUsage, HealthCheckStatus, HealthCheckResult
from hola_server.services.server_service import ServerService
from hola_server.test_utils.fakes import FakeSystemMonitor, FakeServerService
...
class TestServerService:
    ...
    @pytest.fixture
    def server_service(self, mock_context: ServerContext, fake_system_monitor: FakeSystemMonitor) -> ServerService:
    ...
    @pytest.mark.asyncio
    async def test_get_server_status_overall(self, server_service: ServerService, fake_system_monitor: FakeSystemMonitor, mock_context: ServerContext):
        """Test getting server status, ensuring all components are correctly fetched."""
        fake_system_monitor.set_cpu_percent(10.0)
        fake_system_monitor.set_virtual_memory(percent=50.0, available_gb=4.0, total_gb=8.0)
        ...
        mock_context.settings.BUILD_DATE = datetime.now(timezone.utc) - timedelta(days=1)
        ...
        assert status.resources.uptime_seconds == pytest.approx((datetime.now(timezone.utc).timestamp() - fake_boot_time), abs=1.5) # Increased tolerance for boot time comparison
        
        assert isinstance(status.started_at, datetime)
        assert isinstance(status.status_checked_at, datetime)

    @pytest.mark.asyncio
    async def test_get_health_check_healthy(self, server_service: ServerService, fake_system_monitor: FakeSystemMonitor, mock_context: ServerContext):
        ...
        fake_system_monitor.set_virtual_memory(percent=20.0, available_gb=6.0, total_gb=8.0) # Well above thresholds
        fake_system_monitor.set_disk_usage(percent=30.0, free_gb=70.0, total_gb=100.0) # Well above thresholds
        ...
        assert health.checks["file_storage"].status == HealthCheckStatus.HEALTHY # Assumes data_dir is writable

    @pytest.mark.asyncio
    async def test_get_health_check_unhealthy_disk_low_gb(self, server_service: ServerService, fake_system_monitor: FakeSystemMonitor, mock_context: ServerContext):
        """Test getting health check with low disk space (GB threshold)."""
        mock_context.settings.HEALTH_CHECK_DISK_MIN_GB = 1.0
        mock_context.settings.HEALTH_CHECK_DISK_MIN_PERCENT = 5.0
        fake_system_monitor.set_disk_usage(percent=10.0, free_gb=0.5, total_gb=100.0) # free_gb below 1.0 GB
        fake_system_monitor.set_virtual_memory(percent=50.0, available_gb=4.0, total_gb=8.0)
    ...
    @pytest.mark.asyncio
    async def test_get_health_check_unhealthy_disk_low_percent(self, server_service: ServerService, fake_system_monitor: FakeSystemMonitor, mock_context: ServerContext):
        ...
        fake_system_monitor.set_disk_usage(percent=96.0, free_gb=4.0, total_gb=100.0) # 4% free, below 5%
        fake_system_monitor.set_virtual_memory(percent=50.0, available_gb=4.0, total_gb=8.0)
    ...
    @pytest.mark.asyncio
    async def test_get_health_check_unhealthy_memory_low_gb(self, server_service: ServerService, fake_system_monitor: FakeSystemMonitor, mock_context: ServerContext):
        """Test getting health check with low memory (GB threshold)."""
        mock_context.settings.HEALTH_CHECK_MEM_MIN_GB = 0.5 
        mock_context.settings.HEALTH_CHECK_MEM_MIN_PERCENT = 10.0
        fake_system_monitor.set_virtual_memory(percent=50.0, available_gb=0.1, total_gb=8.0) # available_gb below 0.5 GB
        fake_system_monitor.set_disk_usage(percent=50.0, free_gb=50.0, total_gb=100.0)
    ...
    @pytest.mark.asyncio
    async def test_get_health_check_unhealthy_memory_low_percent(self, server_service: ServerService, fake_system_monitor: FakeSystemMonitor, mock_context: ServerContext):
        ...
        fake_system_monitor.set_virtual_memory(percent=95.0, available_gb=0.4, total_gb=8.0) # 5% available, below 10%
        fake_system_monitor.set_disk_usage(percent=50.0, free_gb=50.0, total_gb=100.0)
    ...
    @pytest.mark.asyncio
    async def test_get_version(self, server_service: ServerService, mock_context: ServerContext):
        ...
        mock_context.settings.BUILD_DATE = datetime(2023, 1, 1, 10, 0, 0, tzinfo=timezone.utc)
    ...
    @pytest.mark.asyncio
    async def test_get_resource_usage(self, server_service: ServerService, fake_system_monitor: FakeSystemMonitor, mock_context: ServerContext):
        """Test getting resource usage."""
        fake_system_monitor.set_cpu_percent(25.5)
        fake_system_monitor.set_virtual_memory(percent=45.2, available_gb=4.0, total_gb=8.0)
        ...
        fake_boot_timestamp = (datetime.now(timezone.utc) - timedelta(hours=2, minutes=30)).timestamp()
        ...
        assert usage.uptime_seconds == pytest.approx((datetime.now(timezone.utc).timestamp() - fake_boot_timestamp), abs=1.5)
...
class TestFakeServerService:
    ...
    async def test_get_server_status(self, fake_service: FakeServerService):
        ...
        fake_health = HealthStatus(status=HealthCheckStatus.HEALTHY, checks={}, checked_at=datetime.now(timezone.utc))
        fake_version = VersionInfo(version="fake", build_id="fake", build_date=datetime.now(timezone.utc), git_commit="fake", python_version="3.x")
        fake_resources = ResourceUsage(cpu_percent=10, memory_used_bytes=100, memory_total_bytes=1000, disk_used_bytes=100, disk_total_bytes=1000, uptime_seconds=3600, measured_at=datetime.now(timezone.utc))
    ...
    async def test_get_health_check(self, fake_service: FakeServerService):
        ...
        assert health.status == HealthCheckStatus.HEALTHY 
        assert all(check.status == HealthCheckStatus.HEALTHY for check in health.checks.values())
        assert len(fake_service.method_calls) == 1
