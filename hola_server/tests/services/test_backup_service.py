...
from hola_shared.models import BackupInfo, BackupCreateRequest, RestoreRequest
from hola_shared.errors import NotFoundException, ValidationException, ServiceException
from hola_server.services.backup_service import BackupService
...
class TestBackupService:
    ...
    async def test_restore_backup_success(self, backup_service, temp_dir):
        ...
        restore_response = await backup_service.restore_backup(backup_id, restore_request)
...
class TestFakeBackupService:
    ...
    async def test_list_backups(self, fake_service):
        ...
        # Create some backups
        await fake_service.create_backup("app1", BackupCreateRequest(description="Test backup"))
        await fake_service.create_backup("app1", BackupCreateRequest(description="Another test backup"))
        await fake_service.create_backup("app2", BackupCreateRequest(description="Test backup"))
    ...
    def test_reset(self, fake_service):
        ...
        import asyncio
        asyncio.run(fake_service.create_backup("test-app", BackupCreateRequest(description="Test backup")))
