...
from hola_shared.models.logs import LogEntry, LogQueryParams, LogLevel, LogSource, LogClearRequest, LogCreateRequest
from hola_server.services.log_service import LogService
from hola_server.test_utils.fakes.fake_log_service import FakeLogService
...
class TestLogService:
    ...
    def create_test_log_entry(self, level: LogLevel = LogLevel.INFO, message: str = "Test message", source: LogSource = LogSource.APPLICATION) -> LogEntry:
    ...
    async def test_add_log_entry(self, log_service):
        ...
        from hola_shared.models.logs import LogCreateRequest
    ...
    async def test_get_logs_with_level_filter(self, log_service):
        ...
        await log_service.add_log_entry(app_name, self.create_test_log_entry(level=LogLevel.INFO))
        await log_service.add_log_entry(app_name, self.create_test_log_entry(level=LogLevel.ERROR))
        await log_service.add_log_entry(app_name, self.create_test_log_entry(level=LogLevel.INFO))
    ...
    async def test_get_logs_with_search(self, log_service):
        ...
        await log_service.add_log_entry(app_name, self.create_test_log_entry(message="User login successful"))
        await log_service.add_log_entry(app_name, self.create_test_log_entry(message="Database connection failed"))
        await log_service.add_log_entry(app_name, self.create_test_log_entry(message="User logout"))
    ...
    async def test_get_log_summary(self, log_service):
        ...
        # Add logs with different levels
        await log_service.add_log_entry(app_name, LogEntry(
            id="info1", 
    ...
    async def test_stream_logs(self, log_service):
        ...
        # Add some logs
        for i in range(3):
            await log_service.add_log_entry(app_name, self.create_test_log_entry(message=f"Stream message {i}"))
...
class TestFakeLogService:
    ...
    def create_test_log_entry(self, level: LogLevel = LogLevel.INFO, message: str = "Test message") -> LogEntry:
        """Create a test log entry."""
        return LogEntry(
            id=f"log-{datetime.now(timezone.utc).timestamp()}",
            timestamp=datetime.now(timezone.utc),
    ...
    async def test_add_log(self, fake_service):
        ...
        app_logs = [log for log_id, log in fake_service.logs.items() if log.app_name == "test-app"]
    ...
    async def test_get_logs_with_filters(self, fake_service):
        ...
        # Add logs with different levels
        await fake_service.add_log_entry("test-app", self.create_test_log_entry(level=LogLevel.INFO))
        await fake_service.add_log_entry("test-app", self.create_test_log_entry(level=LogLevel.ERROR))
    ...
    async def test_stream_logs(self, fake_service):
        ...
        # Add some logs
        await fake_service.add_log_entry("test-app", self.create_test_log_entry(message="Stream test"))
