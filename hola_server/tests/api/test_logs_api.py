...
def test_get_logs(client, fake_log_service):
    ...
    # Act
    response = client.get(
        f"/api/apps/{app_name}/logs?limit=10",
...
def test_get_logs_with_filters(client, fake_log_service):
    ...
    # Act - filter by level
    response = client.get(
        f"/api/apps/{app_name}/logs?level=error",
        headers={"X-API-Key": "test-key"}
    )
    ...
    assert response.json()["data"]["entries"][0]["message"] == "Database connection failed"
    
    # Act - filter by source
    response = client.get(
        f"/api/apps/{app_name}/logs?source=system",
        headers={"X-API-Key": "test-key"}
    )
    ...
    assert response.json()["data"]["entries"][0]["message"] == "High memory usage detected"
