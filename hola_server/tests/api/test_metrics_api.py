...
def test_get_specific_metric(client, fake_metrics_service):
    ...
    # Verify service was called
    assert len(fake_metrics_service.method_calls) == 2  # get_metric_series calls get_metrics internally
...
async def test_get_metrics_summary(client, fake_metrics_service):
    ...
    await fake_metrics_service.generate_test_metrics(app_name, metric_count=5, points_per_metric=20)
    
    # Act
    response = client.get(
        f"/api/apps/{app_name}/metrics/summary",
        headers={"X-API-Key": "test-key"}
    )
    ...
    # Verify service was called
    assert len(fake_metrics_service.method_calls) == 2  # 1 for generate_test_metrics, 1 for get_summary_metrics
...
async def test_clear_metrics(client, fake_metrics_service):
    ...
    # Verify service was called
    assert len(fake_metrics_service.method_calls) == 2  # 1 for generate_test_metrics, 1 for clear_metrics
