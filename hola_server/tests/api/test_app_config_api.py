...
from hola_shared.models.config import (
    ConfigCreateRequest, ConfigUpdateRequest, ConfigEntry, AppConfig
)
...
def client_with_fake_app_service(fake_app_service):
    ...
    with patch('hola_server.api.app_config.get_app_service', return_value=fake_app_service):
...
def client_with_fake_config(fake_config_service):
    ...
    with patch('hola_server.config.context.ServerContext.get_config_service', return_value=fake_config_service):
        with TestClient(app) as client:
...
class TestConfigAPI:
    ...
    def test_get_app_config_returns_empty_config_for_new_app(self, client_with_fake_config):
        ...
        assert data["config"]["app_name"] == app_name
        assert len(data["config"]["config"]) == 0
    ...
    def test_get_app_config_returns_existing_config(self, client_with_fake_config):
        ...
        assert data["success"] is True
        assert data["config"]["app_name"] == app_name
        assert len(data["config"]["config"]) == 1
        assert "test_key" in data["config"]["config"]
    ...
    def test_get_app_config_validates_app_name(self, client_with_fake_config):
        ...
        assert "App name cannot be empty" in response.json()["detail"]
    ...
    def test_list_config_entries_returns_all_entries(self, client_with_fake_config):
        ...
        now = datetime.now(timezone.utc)
        entries = {
            "key1": ConfigEntry(key="key1", value="value1", description="Test description", is_secret=False, created_at=now, updated_at=now),
            "key2": ConfigEntry(key="key2", value="value2", description="Test description", is_secret=True, created_at=now, updated_at=now),
        }
        config = AppConfig(
            app_name=app_name,
            config=entries,
    ...
    def test_get_config_entry_returns_specific_entry(self, client_with_fake_config):
        ...
        data = response.json()
        assert data["success"] is True
        assert data["entry"]["key"] == key
        assert data["entry"]["value"] == "test_value"
        assert data["entry"]["description"] == "Test config"
        assert data["entry"]["is_secret"] is True

    def test_get_config_entry_returns_404_for_missing_entry(self, client_with_fake_config):
        ...
        assert f"Configuration entry '{key}' not found" in response.json()["detail"]
    ...
    def test_create_config_entry_creates_new_entry(self, client_with_fake_config):
        ...
        data = response.json()
        assert data["success"] is True
        assert data["entry"]["key"] == "new_key"
        assert data["entry"]["value"] == "new_value"
        assert data["entry"]["description"] == "New configuration"
        assert data["entry"]["is_secret"] is True

    def test_create_config_entry_returns_400_for_duplicate_key(self, client_with_fake_config):
        ...
        # Pre-create config entry
        now = datetime.now(timezone.utc)
        entry = ConfigEntry(key=key, value="value", description="Test description", is_secret=False, created_at=now, updated_at=now)
        config = AppConfig(
            app_name=app_name,
            config={key: entry},
            created_at=now,
        ...
        assert response.status_code == 400
        assert f"Configuration entry '{key}' already exists" in response.json()["detail"]
    ...
    def test_update_config_entry_updates_existing_entry(self, client_with_fake_config):
        ...
        response = client.put(f"/api/config/{app_name}/entries/{key}", json=request_data)
        
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
        assert data["entry"]["key"] == key
        assert data["entry"]["value"] == "updated_value"
        assert data["entry"]["description"] == "Updated description"
        assert data["entry"]["is_secret"] is True

    def test_update_config_entry_returns_404_for_missing_entry(self, client_with_fake_config):
    ...
    def test_delete_config_entry_returns_404_for_missing_entry(self, client_with_fake_config):
    ...
    def test_delete_app_config_removes_all_config(self, client_with_fake_config):
        ...
        entries = {
            "key1": ConfigEntry(key="key1", value="value1", description="Test description", is_secret=False, created_at=now, updated_at=now),
            "key2": ConfigEntry(key="key2", value="value2", description="Test description", is_secret=False, created_at=now, updated_at=now),
        }
        config = AppConfig(
            app_name=app_name,
            config=entries,
    ...
    def test_config_supports_different_value_types(self, client_with_fake_config):
        ...
        for key, value in test_cases:
            ...
            response = client.post(f"/api/config/{app_name}/entries", json=request_data)
            assert response.status_code == 201
            assert response.json()["entry"]["value"] == value
        
        # Verify all entries can be retrieved
        response = client.get(f"/api/config/{app_name}/entries")
        assert response.status_code == 200
        assert response.json()["count"] == len(test_cases)
    ...
    def test_api_error_handling_for_invalid_app_names(self, client_with_fake_config):
        ...
        for invalid_name in invalid_names_404:
            ...
            response = client.post(f"/api/config/{invalid_name}/entries", json=request_data)
            assert response.status_code == 404
    ...
    def test_api_preserves_config_metadata(self, client_with_fake_config):
        ...
        create_response = client.post(f"/api/config/{app_name}/entries", json=request_data)
        assert create_response.status_code == 201
        
        created_at = create_response.json()["entry"]["created_at"]
        ...
        update_response = client.put(f"/api/config/{app_name}/entries/{key}", json=update_data)
        assert update_response.status_code == 200
        
        # Verify metadata is preserved/updated correctly
        entry_data = update_response.json()["entry"]
