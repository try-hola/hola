...
class TestFakeAppServiceFiles:
    ...
    @pytest.mark.asyncio
    async def test_upload_app_file_with_path(self, fake_app_service_with_app, mock_upload_file):
        """Test uploading a file with a specific path."""
        file_info = await fake_app_service_with_app.upload_app_file(
            "test-app",
            mock_upload_file,
            path="folder/custom.txt"
        )
        
        assert file_info.path == "folder/custom.txt"
        
        # Verify file was stored with the custom path
        assert fake_app_service_with_app.file_storage.has_file("test-app", "folder/custom.txt")
    
    @pytest.mark.asyncio
    async def test_upload_app_file_app_not_found(self, fake_app_service, mock_upload_file):
    ...
    @pytest.mark.asyncio
    async def test_upload_app_file_forced_failure(self, fake_app_service_with_app, mock_upload_file):
        """Test forced failure when uploading a file."""
        fake_app_service_with_app.set_failure_mode("file_upload", True)
        
        with pytest.raises(ValidationException) as excinfo:
            await fake_app_service_with_app.upload_app_file("test-app", mock_upload_file)
    ...
    async def test_delete_app_file_not_found(self, fake_app_service_with_app):
        ...
        with pytest.raises(NotFoundException) as excinfo:
            await fake_app_service_with_app.delete_app_file("test-app", "non-existent.txt")
