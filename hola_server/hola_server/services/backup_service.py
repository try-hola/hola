...
class BackupService:
    ...
    async def create_backup(self, app_name: str, request: BackupCreateRequest) -> BackupCreateResponse:
        ...
        try:
            ...
            logger.error(f"Failed to create backup for {app_name}: {str(e)}")
            raise ServiceException(f"Failed to create backup: {str(e)}", service_name="BackupService")
    ...
    async def list_backups(self, app_name: Optional[str] = None) -> BackupListResponse:
        ...
        try:
            ...
            raise ServiceException(f"Failed to list backups: {str(e)}", service_name="BackupService")
    ...
    async def get_backup_info(self, backup_id: str) -> BackupInfo:
        ...
        try:
            ...
            raise ServiceException(f"Failed to get backup info: {str(e)}", service_name="BackupService")
    ...
    async def delete_backup(self, backup_id: str) -> None:
        ...
        try:
            ...
            raise ServiceException(f"Failed to delete backup: {str(e)}", service_name="BackupService")
    
    async def restore_backup(self, backup_id: str, request: RestoreRequest) -> RestoreResponse:
        ...
        try:
            ...
            if backup_info.status != BackupStatus.COMPLETED:
                raise ValidationException(f"Cannot restore from backup in status: {backup_info.status}")
            ...
            raise ServiceException(f"Failed to restore backup: {str(e)}", service_name="BackupService")
    ...
    async def _create_backup_archive(
        ...
        with tarfile.open(archive_path, "w:gz") as tar:
            ...
            metadata = {
                "app_name": app_name,
                "app_data": app.model_dump(mode="json"),  # Use json mode to handle datetime serialization
            ...
            if request.include_config:
                try:
                    ...
                    config_path.write_text(json.dumps(app_config.model_dump(mode="json"), indent=2))
            ...
            if request.include_data:
                try:
                    app_data_dir = Path(self.settings.data_path) / "apps" / app_name / "data"
    ...
    async def _perform_restore(
        self, backup_info: BackupInfo, request: RestoreRequest, restore_info: RestoreInfo
    ) -> None:
        ...
        if not archive_path.exists():
            raise ServiceException(f"Backup archive not found: {archive_path}", service_name="BackupService")
        ...
        try:
            ...
            with tarfile.open(archive_path, "r:gz") as tar:
                tar.extractall(temp_dir, filter='data')  # Use 'data' filter to avoid security warnings
            ...
            if metadata_path.exists():
                ...
                raise ServiceException("Backup metadata not found", service_name="BackupService")
            ...
            if request.restore_files and backup_info.includes_files:
                ...
                target_app_dir = Path(self.settings.data_path) / "apps" / restore_info.target_app_name
            ...
            if request.restore_data and backup_info.includes_data:
                ...
                if data_path.exists():
                    target_data_dir = Path(self.settings.data_path) / "apps" / restore_info.target_app_name / "data"
