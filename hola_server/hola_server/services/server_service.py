...
class ServerService:
    ...
    def __init__(self, context: ServerContext, system_monitor: SystemMetrics | None = None):
    ...
    async def get_resource_usage(self) -> ResourceUsage:
        ...
        try:
            ...
            logger.debug(f"Resource usage retrieved - CPU: {cpu_percent}%, Memory: {memory.percent}%")
    ...
    async def _check_disk_space(self) -> HealthCheckResult:
        ...
        try:
            ...
            if free_gb < min_free_gb or free_percent < min_free_percent:
                status = HealthCheckStatus.UNHEALTHY
                message = f"Low disk space: {free_gb:.2f}GB free ({usage_percent:.1f}% used)"
            else:
    ...
    async def _check_memory_usage(self) -> HealthCheckResult:
        ...
        try:
            ...
            # Determine health status based on thresholds
            if available_gb < min_available_gb or available_percent < min_available_percent:
                status = HealthCheckStatus.UNHEALTHY
