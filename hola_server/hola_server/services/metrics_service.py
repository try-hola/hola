...
class MetricsService:
    ...
    def __init__(self, context: ServerContext):
        ...
        self._metrics: Dict[str, Dict[str, List[MetricPoint]]] = defaultdict(lambda: defaultdict(list))
        self._metric_definitions: Dict[str, Dict[str, MetricDefinition]] = defaultdict(dict)
    ...
    async def record_metric(self, app_name: str, request: MetricRecordRequest) -> None:
        ...
        """
        try:
            logger.debug(f"Recording metric {request.name} for app {app_name}: {request.value}")
            ...
            if len(self._metrics[app_name][request.name]) > self._max_points_per_metric:
                ...
                self._metrics[app_name][request.name] = self._metrics[app_name][request.name][-self._max_points_per_metric:]
    ...
    async def get_metrics(self, app_name: str, params: MetricsQueryParams) -> MetricsListResponse:
        ...
        """
        try:
            logger.debug(f"Getting metrics for app {app_name} with params: {params.model_dump()}")
    ...
    async def get_metric_series(self, app_name: str, metric_name: str, params: MetricsQueryParams) -> MetricSeries:
        ...
        try:
            ...
            if app_name not in self._metrics or metric_name not in self._metrics[app_name]:
                raise NotFoundException(
                    resource_type="metric",
                    resource_id=f"{app_name}:{metric_name}"
                )
            
            metric_series = await self._build_metric_series(app_name, metric_name, params)
            if not metric_series:
                raise NotFoundException(
                    resource_type="metric data",
                    resource_id=f"{app_name}:{metric_name}"
                )
    ...
    async def _clear_metrics_internal(self, request: MetricsClearRequest) -> MetricsClearResponse:
        ...
        try:
            ...
            apps_to_process = [request.app_name] if request.app_name else list(self._metrics.keys())
            ...
            for app_name in apps_to_process:
                ...
                metrics_to_clear = request.metric_names or list(self._metrics[app_name].keys())
                ...
                for metric_name in metrics_to_clear:
                    ...
                    if request.before_time or request.older_than_days:
                        ...
                        if request.older_than_days:
                            cutoff_time = datetime.now(timezone.utc) - timedelta(days=request.older_than_days)
                        
                        if cutoff_time:
                            # Keep points after cutoff time
                            self._metrics[app_name][metric_name] = [
                                point for point in self._metrics[app_name][metric_name]
                    ...
                    if points_cleared > 0:
                        ...
                        if points_after == 0:
                            ...
                            if metric_name in self._metric_definitions[app_name]:
                                self._metric_definitions[app_name][metric_name].total_points = points_after
            ...
            logger.info(f"Cleared {cleared_metrics} metrics and {cleared_data_points} data points")
    ...
    def _generate_metrics_summary(self, app_name: str, params: MetricsQueryParams) -> MetricsSummary:
        ...
        total_data_points = sum(len(points) for points in self._metrics[app_name].values())
            ...
            for metric_name in possible_names:
                if metric_name in self._metrics[app_name]:
                    ...
                    if points:
                        ...
                        if summary_key in ['memory_usage_bytes', 'disk_usage_bytes', 'network_rx_bytes', 'network_tx_bytes']:
                            value = int(value)
        ...
        return MetricsSummary(
            ...
            summary_start_time=params.start_time or datetime.now(timezone.utc) - timedelta(hours=1),
            summary_end_time=params.end_time or datetime.now(timezone.utc),
            **summary_data
        )
    
    def _generate_empty_summary(self, app_name: str, params: MetricsQueryParams) -> MetricsSummary:
