...
from ..config.context import ServerContext # This import will be a circular dependency if FileStorage is initialized in ServerContext directly. Will need to fix later.
...
class FileStorage:
    ...
    def __init__(self, context: ServerContext):
        ...
        self.base_path = os.path.join(self.settings.data_dir, "apps") # Base path for app files
    ...
    async def list_files(self, app_name: str) -> FileListResponse:
            ...
            for root, _, filenames in os.walk(app_files_path):
                for filename in filenames:
                    ...
                    try:
                        ...
                        logger.warning(f"File not found during stat: {full_path} for app '{app_name}'. Skipping.")
                    except Exception as e:
                        logger.error(f"Error stating file {full_path} for app '{app_name}': {e}")
                        # Decide if you want to skip or raise
            return {"files_details": collected_files, "total_size_bytes": current_total_size}
        ...
        for file_detail in details_result["files_details"]:
            processed_files.append(FileInfo(
                ...
                modified_at=datetime.fromtimestamp(file_detail["modified_at_timestamp"], tz=timezone.utc),
    ...
    async def upload_file(self, app_name: str, file_path: str, content: bytes, content_type: Optional[str] = None) -> FileInfo:
        ...
        def _stat_file_sync():
            try:
                ...
                raise ServiceException(message=f"File not found after supposedly successful write: {full_path}", service_name="file_storage")
            except OSError as e:
                raise ServiceException(message=f"Failed to stat file after write: {e}", service_name="file_storage")
    ...
    async def get_file(self, app_name: str, file_path: str) -> Optional[BinaryIO]:
        ...
        def _read_file_sync():
            try:
                ...
                # This exception will be caught by the caller of asyncio.to_thread
                # and re-raised in the main thread.
                raise ServiceException(message=f"Failed to read file from disk: {e}", service_name="file_storage")
    ...
    async def delete_file(self, app_name: str, file_path: str) -> bool:
        ...
        exists = await asyncio.to_thread(os.path.exists, full_path)
        if not exists:
            logger.debug(f"File '{file_path}' not found for app '{app_name}', nothing to delete.")
            return False
        ...
        try:
            ...
            app_base_path = self._get_app_path(app_name) # Cache this to avoid repeated calls in loop condition
            ...
            raise ServiceException(message=f"Failed to delete file or cleanup directories: {e}", service_name="file_storage")
