...
class FakeMetricsService:
    ...
    def register_metric_series(self, series: MetricSeries):
        ...
        if metric_name not in self.metric_definitions[app_name]:
            ...
            self.metric_definitions[app_name][metric_name] = MetricDefinition(
                ...
                labels=[label for point in series.points for label in point.labels.keys()],
                created_at=now,
    ...
    async def get_metrics(
        ...
        if app_name not in self.metrics:
            ...
            return MetricsListResponse(metrics={}, summary=empty_summary, query_params=params)
        
        # Filter by metric name if specified
        metric_names = []
        if params.metric_names:
            # Filter to only include metric names that exist
            metric_names = [name for name in params.metric_names if name in self.metrics[app_name]]
            if not metric_names:
                # Create empty summary
                now = datetime.now(timezone.utc)
                empty_summary = MetricsSummary(
        ...
        return MetricsListResponse(metrics=result_series, summary=summary, query_params=params)
    ...
    async def get_metric_definitions(
        ...
        return MetricDefinitionsResponse(definitions=definitions, total_count=len(definitions))
    ...
    async def get_summary_metrics(self, app_name: str) -> Dict[str, Any]:
        ...
        data_point_count = sum(len(points) for points in self.metrics[app_name].values())
    ...
    async def clear_metrics(
        self, app_name: str, metric_name: Optional[str] = None, before_timestamp: Optional[datetime] = None
    ...
    async def generate_test_metrics(
        ...
        if self._failure_modes.get("generate_test_metrics", False):
            raise Exception(f"Simulated failure in generate_test_metrics for {app_name}")
        ...
        for i in range(min(metric_count, len(sample_metrics))):
            ...
            for j in range(points_per_metric):
                ...
                point_time = now - timedelta(hours=24) + timedelta(
                    seconds=(24 * 3600 * j / points_per_metric)
                )
