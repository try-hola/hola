...
class FakeConfigService:
    ...
    async def get_config_entry(self, app_name: str, key: str) -> ConfigEntryResponse:
        ...
        if key not in config_response.config.config:
            raise NotFoundException(resource_type="configuration entry", resource_id=key)
    ...
    async def create_config_entry(self, app_name: str, request: ConfigCreateRequest) -> ConfigEntryResponse:
        ...
        if key in app_config.config:
            raise ValidationException(f"Configuration entry '{key}' already exists for app '{app_name}'")
    ...
    async def update_config_entry(self, app_name: str, key: str, request: ConfigUpdateRequest) -> ConfigEntryResponse:
        ...
        updated_entry = ConfigEntry(
            key=entry.key,
            value=request.value,
            description=request.description if request.description is not None else entry.description,
            is_secret=request.is_secret,
