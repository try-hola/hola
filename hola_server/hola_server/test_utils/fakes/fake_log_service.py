...
from hola_shared.models.logs import (
    LogEntry, LogLevel, LogSource, LogQueryParams, LogResponse,
    LogSummary, LogCreateRequest, LogClearRequest, LogClearResponse
)
...
class FakeLogService:
    ...
    async def add_log_entry(self, app_name: Optional[str], entry: LogCreateRequest) -> LogEntry:
        """Add a new log entry."""
    ...
    async def get_logs(self, params: LogQueryParams) -> LogResponse:
        ...
        if params.start_time:
            filtered_logs = [log for log in filtered_logs if log.timestamp >= params.start_time]
        if params.end_time:
            filtered_logs = [log for log in filtered_logs if log.timestamp <= params.end_time]
        ...
        # Filter by source
        if params.source:
            filtered_logs = [log for log in filtered_logs if log.source == params.source]
        
        # Filter by app name
        if params.app_name:
            filtered_logs = [log for log in filtered_logs if log.app_name == params.app_name]
        ...
        if params.request_id:
            filtered_logs = [log for log in filtered_logs if log.request_id == params.request_id]
        if params.session_id:
            filtered_logs = [log for log in filtered_logs if log.session_id == params.session_id]
        if params.user_id:
            filtered_logs = [log for log in filtered_logs if log.user_id == params.user_id]
        ...
        for level in LogLevel:
            level_counts[level.value] = sum(1 for log in filtered_logs if log.level == level)
        
        # Count by source
        source_counts = {}
        for source in LogSource:
            source_counts[source.value] = sum(1 for log in filtered_logs if log.source == source)
        
        # Calculate summary
        total_count = len(filtered_logs)
        earliest = min((log.timestamp for log in filtered_logs), default=None) if filtered_logs else None
        latest = max((log.timestamp for log in filtered_logs), default=None) if filtered_logs else None
        ...
        summary = LogSummary(
            ...
            latest_entry=latest,
            size_bytes=total_count * 500  # Mock average size of 500 bytes per log entry
        )
