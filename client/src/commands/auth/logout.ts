  ...
  try {
    if (options.all) {
      ...
      if (serverNames.length === 0) {
        return outputFormatter.formatOutput('info', 'No server contexts found.');
      }
      for (const name of serverNames) {
        await authManager.logout(name);
      }
      return outputFormatter.formatOutput('success', `Logged out from all ${serverNames.length} server contexts.`);
    }
    const serverContext = await configManager.resolveServerContext(options.server);
    if (!serverContext) {
      return outputFormatter.formatOutput('error', 'No server context found. Please specify a valid server with --server.');
    }
    await authManager.logout(serverContext.name);
    outputFormatter.formatOutput('success', `Logged out from server: ${serverContext.name}`);
  } catch (error) {
