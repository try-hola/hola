...
describe('app deploy command', () => {
  ...
  it('should deploy app successfully', async () => {
    ...
    const result = await deployModule.handler('myapp', 'package.tgz', { force: true });
    ...
    expect(consoleLog).toHaveBeenCalledWith(expect.stringContaining('deployed successfully'));
  ...
  it('should handle API errors gracefully', async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(expect.stringContaining('Failed to deploy'), expect.anything());
