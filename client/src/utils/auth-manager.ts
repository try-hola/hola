...
class AuthManager {
  ...
  async authenticate(serverContext) {
    ...
    const authDomain = serverContext.providerOptions.orbDomain.replace(/^[^.]+\./, 'auth.');
    ...
    authorizationUrl.searchParams.append('redirect_uri', 'http://localhost:8888/callback');
    authorizationUrl.searchParams.append('response_type', 'code');
    authorizationUrl.searchParams.append('scope', 'openid profile email offline_access');
    authorizationUrl.searchParams.append('state', state);
    authorizationUrl.searchParams.append('code_challenge', codeChallenge);
    authorizationUrl.searchParams.append('code_challenge_method', 'S256');
    
    // Open browser for auth
    outputFormatter.formatOutput('info', 'Opening browser for authentication...');
    ...
    try {
      ...
      outputFormatter.formatOutput('error', `Authentication failed: ${error.message}`);
      throw error;
    } finally {
      server.close();
    }
  ...
  async getAccessToken(serverName) {
    ...
    if (!tokens) {
      outputFormatter.formatOutput('error', 'No authentication tokens found. Please authenticate first.');
  ...
  async refreshToken(serverName, refreshToken) {
    ...
    try {
      ...
      outputFormatter.formatOutput('error', `Failed to refresh token: ${error.message}`);
  ...
  async getTokens(serverName) {
    const tokens = await keytar.getPassword(this.serviceName, `${serverName}_tokens`);
  ...
  createCallbackServer() {
    ...
    server.listen(8888, () => {
      outputFormatter.formatOutput('info', 'Waiting for authentication callback on port 8888...');
    });
  ...
  async logout(serverName) {
    ...
    outputFormatter.formatOutput('success', `Logged out from server "${serverName}"`);
  }
}
