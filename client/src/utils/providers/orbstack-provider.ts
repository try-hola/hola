...
class OrbStackProvider implements ServerProvider {
  ...
  async bootstrap(options: ServerBootstrapOptions): Promise<ServerContext> {
    ...
    outputFormatter.formatOutput(
      "info",
      "Setting up OIDC authentication with Authentik...",
    );
    ...
    outputFormatter.formatOutput(
      "info",
      "Server bootstrapped successfully. Please authenticate now.",
    );
  ...
  getConfigOptions(): ServerProviderConfigOptions {
    return {
      subdomain: {
        ...
        label: "Subdomain",
        description:
          'The subdomain to use for the server (defaults to "hola" resulting in hola.local)',
        required: false,
        default: "hola",
      },
    };
  ...
  private async deployComposeFile(
    ...
    try {
      ...
      outputFormatter.formatOutput(
        "info",
        "Server components deployed successfully",
      );
    } catch (error: any) {
      outputFormatter.formatOutput(
        "error",
   ...
   */
  private async configureAuthentik(
    domain: string,
    ...
    outputFormatter.formatOutput(
      "info",
      "Configuring Authentik OIDC client...",
    );
    ...
    try {
      ...
      outputFormatter.formatOutput(
        "info",
        `Created OIDC client with ID: ${clientId}`,
      );
      return clientId;
    } catch (error: any) {
      outputFormatter.formatOutput(
        "error",
        `Failed to configure Authentik: ${error.message}`,
      );
      throw error;
    }
  }
