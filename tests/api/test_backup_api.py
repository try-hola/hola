...
from hola.api.backup import get_backup_service
from hola.auth import get_api_key
...
def test_create_backup(client, fake_backup_service):
    ...
    response = client.post(
        f"/api/apps/{app_name}/backups", json=request, headers={"X-API-Key": "test-key"}
    )
...
def test_list_backups(client, fake_backup_service):
    ...
    # Act
    response = client.get(
        f"/api/apps/{app_name}/backups", headers={"X-API-Key": "test-key"}
    )
...
def test_get_backup(client, fake_backup_service):
    ...
    # Act
    response = client.get(
        f"/api/apps/{app_name}/backups/{backup_id}", headers={"X-API-Key": "test-key"}
    )
