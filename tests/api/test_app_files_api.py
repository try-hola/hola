...
def client_with_fake_app_service(fake_app_service_with_app):
    ...
    from hola.api.app_files import get_app_service
...
def mock_api_key():
    ...
    with patch("hola.api.app_files.get_api_key", return_value="test-api-key"):
        yield "test-api-key"
...
class TestAppFilesApi:
    ...
    async def test_list_files_empty(
        self, client_with_fake_app_service, mock_api_key, fake_app_service_with_app
    ):
        ...
        assert fake_app_service_with_app.was_method_called_with(
            "list_app_files", app_name="test-app"
        )
    ...
    @pytest.mark.asyncio
    async def test_list_files_app_not_found(
        self, client_with_fake_app_service, mock_api_key, fake_app_service_with_app
    ):
        ...
        assert fake_app_service_with_app.get_method_call_count("list_app_files") == 1
        assert fake_app_service_with_app.was_method_called_with(
            "list_app_files", app_name="non-existent-app"
        )
    ...
    async def test_upload_file(
        ...
        response = client_with_fake_app_service.post(
            "/api/apps/test-app/files/?path=test-directory/test.txt", files=files
        )
        ...
        assert data["success"] is True
        assert (
            data["data"]["path"] == "test-directory/test.txt"
        )  # Path should reflect the provided path
    ...
    async def test_get_file(
        ...
        assert fake_app_service_with_app.was_method_called_with(
            "get_app_file", app_name="test-app", file_path="test.txt"
        )

    @pytest.mark.asyncio
    async def test_get_file_content_type_and_headers(
        self, client_with_fake_app_service, mock_api_key, fake_app_service_with_app
    ):
        ...
        # Test text file
        text_response = client_with_fake_app_service.get(
            "/api/apps/test-app/files/document.txt"
        )
        assert text_response.status_code == 200
        assert text_response.content == text_content
        assert text_response.headers["content-type"].startswith("text/plain")
        assert "attachment; filename=document.txt" in text_response.headers.get(
            "content-disposition", ""
        )

        # Test image file
        image_response = client_with_fake_app_service.get(
            "/api/apps/test-app/files/photo.jpg"
        )
        assert image_response.status_code == 200
        assert image_response.content == image_content
        assert image_response.headers["content-type"] == "image/jpeg"
        assert "attachment; filename=photo.jpg" in image_response.headers.get(
            "content-disposition", ""
        )

        # Verify get_file_info was called through file_storage (indirectly via API)
        assert (
            fake_app_service_with_app.file_storage.get_method_call_count(
                "get_file_info"
            )
            == 2
        )
    ...
    async def test_get_file_not_found(
        ...
        await fake_app_service_with_app.file_storage.delete_file(
            "test-app", "non-existent.txt"
        )  # Ensure it's clean

        response = client_with_fake_app_service.get(
            "/api/apps/test-app/files/non-existent.txt"
        )
        ...
        assert fake_app_service_with_app.get_method_call_count("get_app_file") == 1
        assert fake_app_service_with_app.was_method_called_with(
            "get_app_file", app_name="test-app", file_path="non-existent.txt"
        )
    ...
    async def test_delete_file(
        ...
        response = client_with_fake_app_service.delete(
            "/api/apps/test-app/files/test.txt"
        )
        ...
        assert fake_app_service_with_app.was_method_called_with(
            "delete_app_file", app_name="test-app", file_path="test.txt"
        )
    ...
    async def test_delete_file_not_found(
        ...
        assert fake_app_service_with_app.get_method_call_count("delete_app_file") == 1
        assert fake_app_service_with_app.was_method_called_with(
            "delete_app_file", app_name="test-app", file_path="non-existent.txt"
        )
