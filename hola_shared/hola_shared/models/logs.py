...
class LogQueryParams(BaseModel):
    ...
    # Pagination
    limit: int = Field(
        default=100, ge=1, le=1000, description="Maximum number of entries to return"
    )
    offset: int = Field(default=0, ge=0, description="Number of entries to skip")

    # Sorting
    sort_order: str = Field(
        default="desc", pattern="^(asc|desc)$", description="Sort order by timestamp"
    )
...
class LogSummary(BaseModel):
    ...
    earliest_entry: Optional[datetime] = Field(
        None, description="Timestamp of earliest log entry"
    )
    latest_entry: Optional[datetime] = Field(
        None, description="Timestamp of latest log entry"
    )
...
class LogClearRequest(BaseModel):
    ...
    app_name: Optional[str] = Field(
        None, description="Clear logs for specific application"
    )
    ...
    source: Optional[LogSource] = Field(
        None, description="Clear logs from specific source"
    )
