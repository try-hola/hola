...
class MetricSeries(BaseModel):
    ...
    # Time range
    start_time: Optional[datetime] = Field(None, description="First data point timestamp")
...
class MetricsQueryParams(BaseModel):
    ...
    # Content filters
    metric_names: Optional[List[str]] = Field(None, description="Filter by specific metric names")
    metric_type: Optional[MetricType] = Field(None, description="Filter by metric type")
    
    # Aggregation options
    aggregate: bool = Field(default=False, description="Whether to aggregate data points")
    aggregation_interval: Optional[str] = Field(None, description="Aggregation interval (e.g., '1m', '5m', '1h')")
    aggregation_function: str = Field(default="avg", pattern="^(avg|min|max|sum|count)$", description="Aggregation function")
    
    # Pagination
    limit: int = Field(default=1000, ge=1, le=10000, description="Maximum number of data points to return")
    offset: int = Field(default=0, ge=0, description="Number of data points to skip")
    
    # Labels filtering
    labels: Dict[str, str] = Field(default_factory=dict, description="Filter by metric labels")
...
class MetricRecordRequest(BaseModel):
    ...
    unit: MetricUnit = Field(default=MetricUnit.COUNT, description="Unit of measurement")
    description: Optional[str] = Field(None, description="Metric description")
    labels: Dict[str, str] = Field(default_factory=dict, description="Additional metric labels")
    timestamp: Optional[datetime] = Field(None, description="Custom timestamp (defaults to current time)")
...
class MetricDefinition(BaseModel):
    ...
    last_recorded_at: Optional[datetime] = Field(None, description="When the metric was last recorded")
    total_points: int = Field(default=0, description="Total number of recorded data points")
...
class MetricsSummary(BaseModel):
    ...
    memory_usage_bytes: Optional[int] = Field(None, description="Current memory usage in bytes")
    disk_usage_bytes: Optional[int] = Field(None, description="Current disk usage in bytes")
    ...
    # Application metrics
    request_count: Optional[int] = Field(None, description="Total number of requests processed")
...
class MetricsListResponse(BaseModel):
    ...
    metrics: Dict[str, MetricSeries] = Field(..., description="Metrics data by metric name")
...
class MetricDefinitionsResponse(BaseModel):
    ...
    definitions: List[MetricDefinition] = Field(..., description="List of metric definitions")
...
class MetricsClearRequest(BaseModel):
    ...
    app_name: Optional[str] = Field(None, description="Clear metrics for specific application")
    metric_names: Optional[List[str]] = Field(None, description="Clear specific metrics")
    before_time: Optional[datetime] = Field(None, description="Clear metrics before this timestamp")
    older_than_days: Optional[int] = Field(None, description="Clear metrics older than this many days")


class MetricsClearResponse(BaseModel):
    """Response from metrics clearing operation."""
    cleared_metrics: int = Field(..., description="Number of metric definitions cleared")
