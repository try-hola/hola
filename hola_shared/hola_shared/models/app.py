...
class App(BaseModel):
    ...
    status: AppStatus = Field(default=AppStatus.UNKNOWN, description="Current application status")
    health: AppHealth = Field(default=AppHealth.UNKNOWN, description="Application health status")
    image: Optional[str] = Field(None, description="Container image used for the application")
    port: Optional[int] = Field(None, description="Port the application is running on")
    environment: Dict[str, str] = Field(default_factory=dict, description="Environment variables")
    created_at: Optional[datetime] = Field(None, description="Application creation timestamp")
...
class AppDeployRequest(BaseModel):
    ...
    name: str = Field(..., description="Unique application name", min_length=1, max_length=50)
    image: str = Field(..., description="Container image to deploy", min_length=1)
    port: Optional[int] = Field(None, description="Port to expose for the application", ge=1, le=65535)
    environment: Dict[str, str] = Field(default_factory=dict, description="Environment variables")
    description: Optional[str] = Field(None, description="Application description", max_length=500)
    version: Optional[str] = Field(None, description="Application version tag", max_length=50)
...
class AppUpgradeRequest(BaseModel):
    ...
    version: Optional[str] = Field(None, description="New application version tag", max_length=50)
    backup_before_upgrade: bool = Field(True, description="Create backup before upgrading")
...
class AppActionResponse(BaseModel):
    ...
    previous_status: AppStatus = Field(..., description="Application status before the action")
    new_status: AppStatus = Field(..., description="Application status after the action")
...
class AppDeployResponse(BaseModel):
    ...
    estimated_duration: Optional[int] = Field(None, description="Estimated deployment time in seconds")
