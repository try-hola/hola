...
class BackupInfo(BaseModel):
    ...
    completed_at: Optional[datetime] = Field(None, description="When the backup was completed")
    error_message: Optional[str] = Field(None, description="Error message if backup failed")
    
    # Backup content metadata
    includes_config: bool = Field(default=True, description="Whether backup includes configuration")
    includes_files: bool = Field(default=True, description="Whether backup includes application files")
    includes_data: bool = Field(default=True, description="Whether backup includes application data")
    
    # Version information
    app_version: Optional[str] = Field(None, description="Application version at backup time")
    server_version: str = Field(..., description="Server version that created the backup")
...
class BackupCreateRequest(BaseModel):
    ...
    include_files: bool = Field(default=True, description="Include application files in backup")
    include_data: bool = Field(default=True, description="Include application data in backup")
...
class RestoreRequest(BaseModel):
    ...
    target_app_name: Optional[str] = Field(None, description="Target application name (defaults to original)")
...
class RestoreInfo(BaseModel):
    ...
    completed_at: Optional[datetime] = Field(None, description="When the restore operation completed")
    error_message: Optional[str] = Field(None, description="Error message if restore failed")
