...
def server_process() -> Generator[multiprocessing.Process, None, None]:
    ...
    proc = multiprocessing.Process(
        target=run_server, args=(TEST_SERVER_PORT, TEST_API_KEY)
    )
    proc.start()
...
def test_cli_config(
    tmp_path_factory,
) -> Generator[Tuple[Path, CliSettings], None, None]:
    """
    ...
    config_dir = (
        tmp_base / "hola"
    )  # Create a hola subdirectory to match CLI expectations
    ...
    settings = CliSettings(
        servers={
            "test-server": ServerConnection(url=TEST_SERVER_URL, api_key=TEST_API_KEY)
        },
...
def run_cli_command(monkeypatch, test_cli_config):
    ...
    def _run_command(command: List[str], expect_error: bool = False) -> str:
        ...
        result = subprocess.run(
            full_command, capture_output=True, text=True, env=os.environ.copy()
        )
        ...
        if expect_error:
            assert (
                result.returncode != 0
            ), "Command succeeded when it was expected to fail"
        else:
...
class TestHelloIntegration:
    ...
    def test_hello_endpoint_direct(self, server_process):
        ...
        response = requests.get(
            f"{TEST_SERVER_URL}/hello/", headers={"X-API-Key": TEST_API_KEY}
        )
