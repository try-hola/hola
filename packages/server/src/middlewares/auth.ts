...
async function verifyToken(req, res, next) {
  ...
  try {
    ...
    if (!issuerUrl) {
      logEvent("SECURITY", "error", "OIDC issuer URL not configured");
    ...
    if (
      !global.jwksCache ||
      !global.jwksCache.keys ||
      Date.now() - global.jwksCache.timestamp > 3600000
    ) {
      try {
    ...
    logEvent("SECURITY", "error", "Token validation error", {
      error: error.message,
    });
