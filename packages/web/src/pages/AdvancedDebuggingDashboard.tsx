...
export default function DebuggingDashboard() {
  ...
  const loadApiCallHistory = useCallback(async () => {
    try {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-900">
        <div className="text-lg text-gray-100">Loading debugging dashboard...</div>
      </div>
    );
  }
            ...
            <div className="bg-gray-800 rounded-lg border border-gray-700">
              ...
              {selectedCall ? (
                <div className="p-4 space-y-4 max-h-96 overflow-y-auto">
                  ...
                  {/* UI Context */}
                  {selectedCall.uiContext && (
                    <div>
                      <h4 className="font-medium text-gray-300 mb-2">UI Context</h4>
                      <div className="text-sm text-blue-300">{selectedCall.uiContext}</div>
                    </div>
                  )}
                </div>
              ) : (
