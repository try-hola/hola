    ...
    .action(async (options: ServerBootstrapOptions) => {
      try {
        ...
        const availableProviders =
          await serverProviderRegistry.getAvailableProviders();

        if (availableProviders.length === 0) {
          return handleError(
            new Error(
              "No server providers available. Please install OrbStack or another supported provider.",
            ),
          );
        }
        ...
        if (options.type) {
          ...
          if (!provider) {
            return handleError(
              new Error(
                `Server provider "${options.type}" not found. Available types: ${availableProviders.map((p) => p.type).join(", ")}`,
              ),
            );
          }
          ...
          if (!(await provider.isAvailable())) {
            return handleError(
              new Error(
                `Server provider "${options.type}" is not available on this system.`,
              ),
            );
          }
        ...
        if (!options.nonInteractive) {
          ...
          const questions = Object.entries(configOptions).map(
            ([key, config]) => ({
        ...
        outputFormatter.formatOutput(
          "spinner",
          `Bootstrapping server with ${provider.displayName}...`,
        );
