...
### Catalog & install
- `hola catalog [query]` — browse/search the app catalog. Rows show `(channels: rc)` when an app offers a version on a channel besides `stable`. Options: `--category`, `--limit`, `--json`.
- `hola install <appId>` — install a catalog app (draft from catalog → validate → finalize → deploy → watch). Pin a version with `--app-version` or inline as `hola install <appId>@<version>` (defaults to the newest release). `--name` sets the deployment name **and** its subdomain (the app is served at `<name>.<base>`; default `<appId>.<base>`); `--as` is an alias of `--name` (if both are given, `--name` wins and a note is printed). Most apps are single-instance — a second install is rejected unless the catalog marks the app multi-instance; pass `--allow-multiple` with a distinct `--name` to run a second copy on purpose (e.g. `hola install webtop --name desk-2 --allow-multiple`). Enable an optional service the app declares with `--profile <key>` (repeatable, or comma-separated — e.g. `hola install postiz --profile elasticsearch`); unknown keys are ignored and, when omitted, the app's default profiles apply. A few apps declare a **capability contract role** that needs privileged access across app boundaries — a backup app has to read every other app's data. Those installs are refused until you consent with `--grant <contract@version>` (repeatable, or comma-separated — e.g. `hola install backrest --grant backup@1`); the error names the exact flag to re-run with. Silence is a refusal, never an implied yes, so a scripted install of such an app must pass the flag explicitly.

  **Release channels** (`--channel <name>`): follow a channel other than the default `stable` (e.g. `rc`) — `latest` resolves to the newest version eligible on that channel. `--channel` on a single-instance app that's already installed on a *different* channel creates a channel-differentiated second copy without needing `--allow-multiple` (still needs a distinct `--name`/`--as`), e.g. `hola install remo --channel rc --as remo-beta` — but only for a channel the catalog actually **publishes** versions of that app on (the ones `hola catalog` lists as `(channels: rc)`). Any other well-formed channel name installs and is followed normally (it tracks the stable releases), yet doesn't count as a separate channel for the single-instance guard: that second copy is rejected naming the unpublished channel and needs `--allow-multiple`. Pinning a pre-release version implies its channel even with no explicit `--channel`, e.g. `hola install remo@0.11.0-rc.1 --name remo-beta` follows `rc`. On success the CLI prints `Following channel: <c>` whenever the deployment doesn't follow `stable`. A channel copy from the catalog starts with **empty data** — it tests that the new version boots, routes and authenticates, not that it migrates your existing data; [try-hola/hola#429](https://github.com/try-hola/hola/issues/429) tracks cloning an existing deployment's data into a new channel copy.

  Options: `--app-version`, `--name`, `--as`, `--channel`, `--set KEY=VALUE` (repeatable), `--profile <key>` (repeatable), `--grant <contract@version>` (repeatable), `--allow-multiple`, `--strict`, `--no-stream`, `--json`.
