...
/**
 * TRAEFIK_DASHBOARD_PASSWORD (F07) is generated by install.sh, which this
 * command runs — so it is auto-managed, and flagging it as a new required key
 * would prompt the operator for a value they are about to be given. Its
 * companion TRAEFIK_DASHBOARD_USER is defaulted by compose.
 */
const DEFAULTED_OR_GENERATED = new Set([
...
function isAutoManaged(key: string): boolean {
  return AUTO_MANAGED.test(key) || DEFAULTED_OR_GENERATED.has(key);
}
