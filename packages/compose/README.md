...
Traefik's dashboard is `api@internal` answering for itself: it lists every route, service,
middleware and TLS setting on the host — including the hostname of every installed app — and
**Hola's own API authentication is not on that request path**. So the server publishes a router
for it only when it has a credential of its own:

- `TRAEFIK_DASHBOARD_PASSWORD` — generated by `install.sh` whenever `TRAEFIK_DASHBOARD_DOMAIN`
  is set. Read it back out of `.env`.
- `TRAEFIK_DASHBOARD_USER` — defaults to `admin`.

The router then carries a Traefik `basicAuth` middleware, so an unauthenticated request gets a
401 and can inspect nothing. With no password set, **no router is emitted at all** and the server
logs a warning at startup naming the key to set; the hostname stays reserved either way, so no
deployed app can claim it in the meantime. Because `hola update` re-runs the installer, a host
that set a dashboard domain before this change keeps its dashboard across the upgrade — now
behind Basic auth — rather than silently losing it. To turn the dashboard off entirely, leave
`TRAEFIK_DASHBOARD_DOMAIN` blank.

Basic auth (rather than the Authentik forward-auth used for apps) is deliberate: it works
identically with `HOLA_AUTH_MODE=none`, needs no running IdP, and the dashboard is a read-only
diagnostic behind TLS. If you want network-level restriction as well, add a Traefik `ipAllowList`
middleware in your own file-provider config — note it is not a substitute for the credential
behind a reverse proxy or CGNAT.

The **development overlay** (`docker-compose.dev.yml`) still exposes the dashboard unauthenticated
over plain HTTP. That is opt-in, local, and `/etc/hosts`-scoped; do not copy its labels onto a
host that answers on a real domain.
